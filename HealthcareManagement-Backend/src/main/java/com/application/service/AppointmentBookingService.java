...
public class AppointmentBookingService {
	...
	public void saveSlots(Slots slot) {
		...
		slotBookingRepository.saveSlots(val, slot.getEmail(), slot.getDoctorname(), slot.getSpecialization(),
				slot.getAmslot(), "unbooked", slot.getNoonslot(), "unbooked", slot.getPmslot(), "unbooked",
				slot.getDate(), slot.getPatienttype());
	}
	...
	// new add for delete slot
	public void deleteSlotByEmailAndDate(String email, String date) {
		slotBookingRepository.deleteByEmailAndDate(email, date);
	}
}
